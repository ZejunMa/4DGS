...
class ParamGroup:
    def __init__(self, parser: ArgumentParser, name : str, fill_none = False):
        ...
        for key, value in vars(self).items():
            ...
            if key.startswith("_"):
                ...
                key = key[1:]
            t = type(value)
            value = value if not fill_none else None # fill_none == False
            if shorthand:
                if t == bool:
    ...
    # 多态，从args中读取lp/op/pp参数类的所有成员组，返回一个字典，可能是lp/op/pp任意一个类的字典，调用extract之前参数和yaml已经整合完
...
# 处理Loading Parameter 的类，继承ParamGroup, 分为shorthand和not shorthand 两类， 成员变量会被分成是否有shorthand notation加入到parameters里面
